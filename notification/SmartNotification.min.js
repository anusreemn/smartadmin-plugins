...
var ExistMsg = 0,
  SmartMSGboxCount = 0,
  PrevTop = 0;
$.SmartMessageBox = function(a, b,triggerRemove) {
  var c, d;
  a = $.extend(
    {
      title: "",
  ...
  (SmartMSGboxCount += 1),
    0 == ExistMsg &&
      ...
      (c =
        ...
        (a.customButton ? " customBtn" : "") + "' id='MsgBoxBack'></div>"),
      $("body").append(c),
  ...
  for (var i = 0; i <= a.buttons.length - 1; i++)
    (cbClass = (a.customButton && k === 0) ? "btn-primary" : "btn-default"),
    "[" == a.buttons[i]
      ? (j = "")
      : "]" == a.buttons[i]
      ? ((k += 1),
        (j =
          ...
          "-Msg" +
          SmartMSGboxCount +
          "' class='btn "+ cbClass +" btn-sm botTempo'> " +
          j +
          "</button>"),
        (d += j))
      : (j += a.buttons[i]);
  (d += "</div>"),
    ...
    $(".botTempo").click(function(evt) {
      var e = $(this).attr("id"),
        c = e.substr(e.indexOf("-") + 1),
        d = $.trim($(this).text());
        var q;
      if (1 == h) {
      ...
      if (!a.customButton) {
        removeBox()
        } else if (a.customButton) {
            q instanceof Promise ? q.then(res => {
                if (res) {
                  removeBox();
                }
            }) : removeBox();
